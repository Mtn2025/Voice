...
async def main():
    print("🚀 Starting EXHAUSTIVE Advanced Tab Simulation (Post-Fix)...")
    ...
    test_cases = [
        # EXISTING
        ("silenceTimeoutMs", 1200, "Assistant Patience (Silence Timeout)"),
        
        # ADDED VIA MIGRATION
        ("noiseSuppressionLevel", "Low", "Noise Suppression"),
        ("audioCodec", "OPUS", "Audio Codec"),
        ("enableBackchannel", False, "Backchanneling (Escucha Activa)"),
        ("maxDurationSeconds", 900, "Max Duration (Safety)"),
        ("maxRetries", 3, "Max Retries (Safety)"),
        ("inactivityMessage", "¿Hola? ¿Sigues ahí?", "Inactivity Message"),
    ]
    ...
    for ui_key, value, label in test_cases:
        ...
        if success:
            if isinstance(info, dict) and info.get("updated", 0) > 0:
                ...
                note = f"Ignorado (Key '{ui_key}' sin columna)"
        else:
            note = f"Error: {info}"
    ...
    print("\n" + "="*100)
    print(f"{'CONTROL':<35} | {'KEY (FE)':<25} | {'SAVED':<10} | {'VERIFIED':<10} | {'NOTES'}")
    print("-" * 100)
    for res in results:
        key = next((k for k, v, l in test_cases if l == res["Control"]), "N/A")
        print(f"{res['Control']:<35} | {key:<25} | {res['Saved']:<10} | {res['Verified']:<10} | {res['Notes']}")
    print("="*100)
