...
from app.domain.exceptions.tool_exceptions import ToolExecutionError, ToolTimeoutError
...
class APIToolAdapter(ToolPort):
    ...
    async def _fetch_property_data(self, address: str) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Mock response
            # In production: async with session.get(f"{self._api_base_url}/property/price", ...)
            mock_response = {
                "address": address,
